...
int bit_isset_flip(char *array, int index);
void bit_flip(char *array, int index);
int bd_initfree_left_right(int k, int bi, char lr);
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_flip(bd_sizes[k].alloc, blk_index(k, p)); // Lab: Allocator for xv6
  for (; k > fk; k--)
  {
    ...
    bit_flip(bd_sizes[k - 1].alloc, blk_index(k - 1, p)); // Lab: Allocator for xv6
...
void bd_free(void *p)
{
  ...
  for (k = size(p); k < MAXSIZE; k++)
  {
    ...
    if (bit_isset_flip(bd_sizes[k].alloc, buddy) == 1)
    {        // is buddy allocated?
      break; // break out of loop
    }
    // end for Lab: Allocator for xv6
...
void bd_mark(void *start, void *stop)
{
  ...
  for (int k = 0; k < nsizes; k++)
  {
    ...
    for (; bi < bj; bi++)
    {
      ...
      bit_flip(bd_sizes[k].alloc, bi);  // Lab: Allocator for xv6
    }
  }
}
...
int bd_initfree(void *bd_left, void *bd_right)
{
  ...
  { // skip max size
    ...
    free += bd_initfree_left_right(k, left, 0);  // Lab: Allocator for xv6
    if (right <= left)
      continue;
    //free += bd_initfree_pair(k, right);
    free += bd_initfree_left_right(k, right, 1); // Lab: Allocator for xv6
  }
  return free;
}
...
void bd_init(void *base, void *end)
{
  ...
  for (int k = 0; k < nsizes; k++)
  {
    ...
    sz = sizeof(char) * ROUNDUP(NBLK(k), 16) / 16;  // Lab: Allocator for xv6: for only one bit for each pair of blocks
    bd_sizes[k].alloc = p;
...
int bit_isset_flip(char *array, int index)
{
  int index_compress = index >> 1;  // compress two buddy bits to one bit
  char b = array[index_compress / 8];
  char m = (1 << (index_compress % 8));
  return (b & m) == m;
}

// flip the bit at position index in array
void bit_flip(char *array, int index)
{
  int index_compress = index >> 1;  // compress two buddy bits to one bit
  char b = array[index_compress / 8];
...
// If a bd_left is marked as free and the buddy is allocated, put the
// bd_left on the free list at size k.
// If a bd_right is marked as allocated and the buddy is free, put the
// buddy on the free list at size k.
// lr = 0 means bd_left and lr = 1 means bd_right
int bd_initfree_left_right(int k, int bi, char lr)
{
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (bit_isset_flip(bd_sizes[k].alloc, bi))
  {
    // one of the pair is free
    free = BLK_SIZE(k);
    if (lr) // buddy is free
